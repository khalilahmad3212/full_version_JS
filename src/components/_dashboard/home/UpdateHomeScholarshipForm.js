...
export default function UpdateHomeScholarshipForm({ isEdit, currentProduct: currentSlider }) {
  const { enqueueSnackbar } = useSnackbar();
  const [data, setData] = useState();

  const NewBlogSchema = Yup.object().shape({
    para: Yup.string().required('Scholarship Text is required')
  });
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.log(result);
        setFieldValue('para', result.value);
        setData(result);
      } catch (error) {
