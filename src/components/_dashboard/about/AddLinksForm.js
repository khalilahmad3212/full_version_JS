...
export default function AddLinksForm({ data, onChange, isEdit, label1, label2, buttonText }) {
  ...
  const addParagraph = () => {
    setFieldValue('facts', [...values.facts, { link: '', text: '' }]);
                    ...
                    <div key={index}>
                      <Stack direction="column" gap={2} justifyContent="flex-end" sx={{ position: 'relative' }}>
                        ...
                        <TextField
                          ...
                          label={label1 ?? `Fact ${index + 1} text`}
                          variant="outlined"
                        />
                        <TextField
                          key={index}
                          ...
                          onChange={(event) => handleParagraphChange(index, event)}
                          label={label2 ?? `Fact ${index + 1} link`}
                          variant="outlined"
                        />
                      </Stack>
                    </div>
